...
- If the returned output starts with `[antigravity-rescue] ... pool at NN%, running on ...` or `... pool exhausted mid-run, reran on ...`, the subagent already switched pools once; pass the result through as-is. If it starts with `[antigravity-rescue] both Antigravity pools exhausted`, nothing ran — hand the task to another delegate or take it inline, and say so once. Do not retry automatically.
